...
public class Levels {
    ...
    public void createExpLevelArray(final float stepSize, float startValue) {
        if (!map.containsKey(new Float(stepSize))) {
            ...
            try {
                ...
                float stepSizeTemp = 1 + (stepSize / 100); // 1.01, 1.02, 1.03,
                                                           // 1.04
    ...
    public void createNormalLevelArray(final float stepSize, float startValue) {
        if (!map.containsKey(new Float(stepSize))) {
            float values[] = new float[30000];
            ;
            int j = 1;
            try {
                ...
                System.out.println("in creeerNivosNormal(), array nivos te klein" + j);
            }
            map.put(new Float(stepSize), values);
        }
    }
